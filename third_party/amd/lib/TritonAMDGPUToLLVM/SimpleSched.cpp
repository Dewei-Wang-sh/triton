...
struct SimpleSched : public FunctionPass {
  static char ID;
  SimpleSched() : FunctionPass(ID) {}
      ...
      for (auto &Region : Regions) {
        ...
        for (Instruction *MMA : MMAs) {
          ...
          while (!localLoads.empty()) {
            ...
            if (loadUsers.count(curr) ||
                !RegionSet.count(curr))
              continue;
...
// static RegisterPass<MoveLocalLoadToMMA>
//     X("move-local-load-to-mma", "Move each local load just before its MMA
//     use");
char SimpleSched::ID = 0;

namespace mlir::triton::AMD {
void runSimpleSched(Function &F) {
  SimpleSched pass;
