...
#blocked = #ttg.linear<{register = [[0, 0, 1], [0, 0, 2], [0, 0, 4], [0, 4, 0]], lane = [[0,0,8], [0, 0, 16], [0, 0, 32],[1,0,0], [0, 8, 0], [0, 16, 0]], warp = [[0, 1, 0], [0, 2, 0]], block = []}>
...
// shared == shareda
#shared = #ttg.padded_shared<[512:+16] {offset = [[0,0,1], [0,0,2], [0,0,4], [0,0,8], [0,0,16], [0,0,32], [1,0,0], [0,8,0], [0, 16,0], [0, 1, 0], [0, 2, 0], [0, 4, 0]], block = []}>
#shareda = #ttg.padded_shared<[512:+16] {offset = [[0,0,1], [0,0,2], [0,0,4], [0,0,8], [0,0,16], [0,0,32], [1,0,0], [0,8,0], [0, 16,0], [0, 1, 0], [0, 2, 0], [0, 4, 0]], block = []}>
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.shared = 32768 : i32, ttg.target = "hip:gfx950", "ttg.threads-per-warp" = 64 : i32} {
  ...
  ^bb5:  // pred: ^bb0
    ...
    %80 = ttg.local_alloc {allocation.offset = 0 : i32} : () -> !ttg.memdesc<2x2x32x64xbf16, #shareda, #smem, mutable>
    ...
    %137 = ttg.memdesc_index %80[%c0_i32] : !ttg.memdesc<2x2x32x64xbf16, #shareda, #smem, mutable> -> !ttg.memdesc<2x32x64xbf16, #shareda, #smem, mutable>
    ...
    %138 = amdg.buffer_load_to_local %a_ptr[%136] mask = %tile_id_c_19 into %137 {contiguity = 8 : i32} : <bf16>[tensor<2x32x64xi32, #blocked>]  -> <2x32x64xbf16, #shareda, #smem, mutable>
    ...
    cf.br ^bb6(%c0_i32, %c0_i32, %0, %16, %cst, %c0_i32, %139, %151, %c0_i32, %137, %149, %103, %101, %115, %113 : i32, i32, i32, i32, tensor<64x64xf32, #mma>, i32, !ttg.async.token, !ttg.async.token, i32, !ttg.memdesc<2x32x64xbf16, #shareda, #smem, mutable>, !ttg.memdesc<2x32x64xbf16, #shared, #smem, mutable>, tensor<2x32x64xi64, #blocked>, tensor<2x32x64xi1, #blocked>, tensor<2x32x64xi64, #blocked>, tensor<2x32x64xi1, #blocked>)
  ^bb6(%tile_id_c_22: i32 loc("tile_id_c"), %152: i32, %153: i32, %154: i32, %155: tensor<64x64xf32, #mma>, %156: i32, %157: !ttg.async.token, %158: !ttg.async.token, %159: i32, %160: !ttg.memdesc<2x32x64xbf16, #shareda, #smem, mutable>, %161: !ttg.memdesc<2x32x64xbf16, #shared, #smem, mutable>, %162: tensor<2x32x64xi64, #blocked>, %163: tensor<2x32x64xi1, #blocked>, %164: tensor<2x32x64xi64, #blocked>, %165: tensor<2x32x64xi1, #blocked>):  // 2 preds: ^bb5, ^bb15
  ...
  ^bb11:  // pred: ^bb10
    ...
    %217 = ttg.memdesc_index %80[%tile_id_c_30] : !ttg.memdesc<2x2x32x64xbf16, #shareda, #smem, mutable> -> !ttg.memdesc<2x32x64xbf16, #shareda, #smem, mutable>
    %218 = amdg.buffer_load_to_local %a_ptr[%216] mask = %211 into %217 {contiguity = 8 : i32} : <bf16>[tensor<2x32x64xi32, #blocked>]  -> <2x32x64xbf16, #shareda, #smem, mutable>
    %219 = ttg.async_commit_group tokens %218
    %220 = ttg.local_load %160 token %166 : !ttg.memdesc<2x32x64xbf16, #shareda, #smem, mutable> -> tensor<2x32x64xbf16, #linear1>
  ...
  ^bb15:  // pred: ^bb14
    ...
    cf.br ^bb6(%tile_id_c_41, %tile_id_c_27, %tile_id_c_34, %tile_id_c_40, %tile_id_c_39, %tile_id_c_30, %219, %229, %tile_id_c_32, %217, %227, %tile_id_c_35, %tile_id_c_36, %tile_id_c_37, %tile_id_c_38 : i32, i32, i32, i32, tensor<64x64xf32, #mma>, i32, !ttg.async.token, !ttg.async.token, i32, !ttg.memdesc<2x32x64xbf16, #shareda, #smem, mutable>, !ttg.memdesc<2x32x64xbf16, #shared, #smem, mutable>, tensor<2x32x64xi64, #blocked>, tensor<2x32x64xi1, #blocked>, tensor<2x32x64xi64, #blocked>, tensor<2x32x64xi1, #blocked>)
  ^bb16:  // pred: ^bb6
    %tile_id_c_42 = arith.cmpi sge, %tile_id_c_15, %c1_i32 : i32
    %276 = amdg.async_wait %157, %158 {num_inst = 0 : i32}
    %277 = ttg.local_load %160 token %276 : !ttg.memdesc<2x32x64xbf16, #shareda, #smem, mutable> -> tensor<2x32x64xbf16, #linear1>
  ...
  ^bb22:  // 2 preds: ^bb20, ^bb21
    ...
    ttg.local_dealloc %80 : !ttg.memdesc<2x2x32x64xbf16, #shareda, #smem, mutable>
    cf.br ^bb23
