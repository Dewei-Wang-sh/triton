...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx950", "ttg.threads-per-warp" = 64 : i32} {
  tt.func public @matmul_kernel_reshape(%a_ptr: !tt.ptr<f16> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32} loc("a_ptr"), %b_ptr: !tt.ptr<f16> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32} loc("b_ptr"), %c_ptr: !tt.ptr<f16> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32} loc("c_ptr"), %M: i32 {tt.divisibility = 16 : i32} loc("M"), %N: i32 {tt.divisibility = 16 : i32} loc("N"), %K: i32 {tt.divisibility = 16 : i32} loc("K")) attributes {noinline = false} {
    ...
    %cst_8 = arith.constant dense<0.000000e+00> : tensor<2x32x64xf16, #blocked>
    ...
    %32 = tt.splat %a_ptr : !tt.ptr<f16> -> tensor<2x32x64x!tt.ptr<f16>, #blocked>
    ...
    %38 = tt.splat %b_ptr : !tt.ptr<f16> -> tensor<2x32x64x!tt.ptr<f16>, #blocked>
    ...
    %48 = tt.splat %c_ptr : !tt.ptr<f16> -> tensor<64x64x!tt.ptr<f16>, #blocked1>
    ...
    scf.if %accumulator {
      %tile_id_c = scf.for %tile_id = %0 to %7 step %c4_i32 iter_args(%tile_id_c_13 = %16) -> (i32)  : i32 {
        ...
        %82 = tt.addptr %48, %81 : tensor<64x64x!tt.ptr<f16>, #blocked1>, tensor<64x64xi64, #blocked1>
        %83 = ttg.convert_layout %82 : tensor<64x64x!tt.ptr<f16>, #blocked1> -> tensor<64x64x!tt.ptr<f16>, #linear>
        %84 = ttg.convert_layout %77 : tensor<64x64xi1, #blocked1> -> tensor<64x64xi1, #linear>
        tt.store %83, %cst, %84 : tensor<64x64x!tt.ptr<f16>, #linear>
        scf.yield %50 : i32
      } {tt.flatten}
    } else {
      ...
      %tile_id_c_19:9 = scf.for %tile_id_c_20 = %c0_i32 to %tile_id_c_15 step %c1_i32 iter_args(%arg7 = %c0_i32, %tile_id_c_21 = %tile_id_c_16, %arg9 = %16, %arg10 = %c0_i32, %arg11 = %cst_9, %arg12 = %cst_1, %arg13 = %cst_0, %arg14 = %cst_1, %arg15 = %cst_0) -> (i32, i32, i32, i32, tensor<64x64xf32, #mma>, tensor<2x32x64xi1, #blocked>, tensor<2x32x64xi64, #blocked>, tensor<2x32x64xi1, #blocked>, tensor<2x32x64xi64, #blocked>)  : i32 {
        ...
        %64 = tt.addptr %32, %63 : tensor<2x32x64x!tt.ptr<f16>, #blocked>, tensor<2x32x64xi64, #blocked>
        %65 = tt.load %64, %60, %cst_8 {loop.cluster = 0 : i32, loop.stage = 0 : i32} : tensor<2x32x64x!tt.ptr<f16>, #blocked>
        %66 = ttg.convert_layout %65 : tensor<2x32x64xf16, #blocked> -> tensor<2x32x64xf16, #linear1>
        %67 = tt.reshape %66 : tensor<2x32x64xf16, #linear1> -> tensor<64x64xf16, #ttg.dot_op<{opIdx = 0, parent = #mma, kWidth = 8}>>
        ...
        %71 = tt.addptr %38, %70 : tensor<2x32x64x!tt.ptr<f16>, #blocked>, tensor<2x32x64xi64, #blocked>
        %72 = tt.load %71, %68, %cst_8 {loop.cluster = 0 : i32, loop.stage = 0 : i32} : tensor<2x32x64x!tt.ptr<f16>, #blocked>
        %73 = ttg.convert_layout %72 : tensor<2x32x64xf16, #blocked> -> tensor<2x32x64xf16, #linear2>
        %74 = tt.reshape %73 : tensor<2x32x64xf16, #linear2> -> tensor<64x64xf16, #linear3>
        %75 = tt.trans %74 {order = array<i32: 1, 0>} : tensor<64x64xf16, #linear3> -> tensor<64x64xf16, #ttg.dot_op<{opIdx = 1, parent = #mma, kWidth = 8}>>
        %76 = tt.dot %67, %75, %arg11 {loop.cluster = 1 : i32, loop.stage = 1 : i32} : tensor<64x64xf16, #ttg.dot_op<{opIdx = 0, parent = #mma, kWidth = 8}>> * tensor<64x64xf16, #ttg.dot_op<{opIdx = 1, parent = #mma, kWidth = 8}>> -> tensor<64x64xf32, #mma>
        ...
        %tile_id_c_28 = scf.if %tile_id_c_26 -> (i32) {
          ...
          %88 = arith.truncf %76 : tensor<64x64xf32, #mma> to tensor<64x64xf16, #mma>
          ...
          %110 = tt.addptr %48, %109 : tensor<64x64x!tt.ptr<f16>, #blocked1>, tensor<64x64xi64, #blocked1>
          %111 = ttg.convert_layout %110 : tensor<64x64x!tt.ptr<f16>, #blocked1> -> tensor<64x64x!tt.ptr<f16>, #linear>
          %112 = ttg.convert_layout %88 : tensor<64x64xf16, #mma> -> tensor<64x64xf16, #linear>
          %113 = ttg.convert_layout %105 : tensor<64x64xi1, #blocked1> -> tensor<64x64xi1, #linear>
          tt.store %111, %112, %113 : tensor<64x64x!tt.ptr<f16>, #linear>
          scf.yield %77 : i32
        } else {
          scf.yield %arg9 : i32
        }
